...
#import adapool_cuda
#from adaPool import EDSCWPool2d

def lip2d(x, logit, kernel, stride, padding):
    weight = logit.exp()
...
class Dsc(nn.Module):
    ...
    def forward(self, x):
        frac = F.avg_pool2d(x, kernel_size =1, stride=1, padding=0)-F.avg_pool2d(x, kernel_size=3, stride=1, padding=1)
        return frac
...
class EMPool2d(nn.Module):
    ...
    def forward(self, x):
        frac = lip2d(x, self.logit(x), self.kernel, self.stride, self.padding)
        return frac

class EDSCWPool2d(nn.Module):
    def __init__(self, kernel_size, stride, padding = 0):
...
class InceptionA(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branchpool = nn.Sequential(
            ...
            EDSCWPool2d(kernel_size=3, stride=1, padding=1),
...
class ReductionA(nn.Module):
    ...
    def __init__(self, input_channels, k, l, m, n):
        ...
        self.branchpool = EMPool2d(kernel_size=3, stride=2)
...
class ReductionB(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branchpool = EMPool2d(kernel_size=3, stride=2, padding=1)
        # self.branchpool = nn.Sequential(
        #     nn.ZeroPad2d(padding=(1, 1, 1, 1)),
        #     EMPool2d(kernel_size=3, stride=2)
        # )
