...
def correct(bam_dir, txt_dir, ec_dir, c_reads_dir, chr):
    ...
    out_read = gzip.open(c_reads_dir+'/'+chr+'.ec.fasta.gz','wt')  #修改后的read
            ...
            for line_txt in lines_txt:
                ...
                if len(flag_del) != 0:
                    for i in range(len(flag_del)):
                        ...
                        out_read.writelines(lines_read[flag_del[len(flag_del)-1-i]*2:flag_del[len(flag_del)-1-i]*2+2])
                        del lines_read[flag_del[len(flag_del)-1-i]*2:flag_del[len(flag_del)-1-i]*2+2]
                    flag_del.clear()
