...
def correct(bam_dir, txt_dir, ec_dir, c_reads_dir, chr):
    ...
    for k in range(len(txt_list)):
        ...
        for bam in bamfile:
            ...
            elif bam.reference_name == refs[k+1]:
                break
        bamfile.close()
        print('bam文件长度为:',len(bamlines))
        print('共有read',len(lines_read)/4)
        ...
        with open(txt_dir +'/'+ txt_list[k],'r') as lines_txt:
            ...
            for line_txt in lines_txt:
                ...
                if ref_ != line_txt_arr[0] and ref_ != '':
                ...
                if int(line_txt_arr[1]) > int(line_ec_arr[1]) and line_txt_arr[0] == line_ec_arr[0]:
                ...
                for i in range(b_len):
                    ...
                    if l_b >= b_len:
                        break
                if int(line_txt_arr[1]) > int(line_ec_arr[1]):
                    print('error',line_txt_arr[1],line_ec_arr[1])
                    break
                ...
                while line_txt_arr[1] == line_ec_arr[1] and line_txt_arr[0] == line_ec_arr[0]:
                    ...
                    num = pos[int(line_ec_arr[2])-1][0] #num为bam中对应read的编号,0开始
                    #bamfile = pysam.AlignmentFile(path_bam, "rb")
                    #numm = 0
                    read_name = ''
                        ...
                        if read_location + pos[int(line_ec_arr[2])-1][1] >= len(str_list) or read_location + pos[int(line_ec_arr[2])-1][1] < 0 :
                            ...
                            if line_ec_arr[4] == '*':
                                ...
                                if (lenth > 5):
                                    ...
                                    read_location += 1
                                    for j in range(lenth):
                                        del str_list[read_location + pos[int(line_ec_arr[2])-1][1]]
                                        del str_list_next[read_location + pos[int(line_ec_arr[2])-1][1]]   
                                    pos[int(line_ec_arr[2])-1][1] -= lenth 
